...
class VistaTextual {
  ...
  Respuestas comprar() {
      ...
      lista.add("SI");
      lista.add("NO");
      String titulo = "¿Queire comprar esta calle?";
      
      int respuesta = menu(titulo, lista);
      if (respuesta == 0){
  ...
  void gestionar () {
      ...
      for(int i=0;i<juegoModel.getJugadorActual().getPropiedades().size();i++){
          TituloPropiedad propiedad = juegoModel.getJugadorActual().getPropiedades().get(i);
          listapropiedades.add(propiedad.getNombre());
      }
      
      iGestion = menu(titulo, lista);
      iPropiedad = menu("Propiedades: ", listapropiedades);
  }
  ...
  void mostrarSiguienteOperacion(OperacionesJuego operacion) {
      System.out.println("La siguiente operación será: " +operacion);
  }
